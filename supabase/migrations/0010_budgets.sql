...
-- ─── budgets: one active cap per (user, category) ────────────────────────────
create table if not exists budgets (
  id            uuid        primary key default gen_random_uuid(),
  user_id       uuid        not null references auth.users (id) on delete cascade,
  category      text        not null,                 -- Plaid PRIMARY string (the key we humanize)
...
-- one ACTIVE budget per category per user (a re-set updates the same row)
create unique index if not exists uniq_budgets_user_category_active
...
create policy budgets_select_own on budgets for select using (auth.uid() = user_id and deleted_at is null);
