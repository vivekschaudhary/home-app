...
import {
  ...
  AAL2_RENEWAL_WINDOW_SECONDS,
  AAL2_TTL_SECONDS,
  aal2CookieOptions,
  aal2RenewAfterSeconds,
  aal2RenewalWindowSeconds,
  aal2TtlSeconds,
...
// Constants + the preview-gated session-clock overrides live in ./aal2-constants
// (crypto-free, Edge-safe) so the Node seam (here, ./guard) and the Edge seam
// (./aal2-edge, ./middleware) read the SAME effective TTL — re-exported here for
// back-compat (the package barrel re-exports this module).
export {
  AAL2_COOKIE,
  AAL2_TTL_SECONDS,
  AAL2_RENEW_AFTER_SECONDS,
...
export function shouldRenewAal2Token(
  ...
  // Derive issuance from the EFFECTIVE TTL the token was minted under, and compare
  // against the EFFECTIVE half-life — so a preview/dev clock-compression shrinks
  // the renewal trigger in lockstep with the mint (Node + Edge read the same TTL).
  const issuedAt = payload.exp - aal2TtlSeconds();
  return nowSeconds - issuedAt >= aal2RenewAfterSeconds();
}
...
export function renewedAal2Cookie(
  ...
  // Re-mint under the effective TTL and stamp the cookie's maxAge to match, so a
  // compressed clock renews to a compressed marker (never stretches it back to 1h).
  const ttl = aal2TtlSeconds();
  return {
    name: AAL2_COOKIE,
    value: signAal2Token(userId, sid, secret, ttl, nowSeconds),
    options: aal2CookieOptions(secure, ttl),
  };
}
