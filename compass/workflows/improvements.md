...
**Next retro fires after improvement #35.** (Retro #006 fired at #30 ON TIME 2026-06-02. **v0.3.14 = #31 (1 of 5 needed before Retro #007).** Hard line still in effect — if Retro #007 slips, it's a 2nd retro-cadence regression; treat as recurring soft-spec failure and deepen `[hard-line-declaration]` mechanism (e.g., make counter visible in dashboard Actions tab).)
...
- **home-app fix (#36 → PR #38):** the missing real-path tests were written FIRST (a jsdom component test, red→green, + the real session→render E2E) — and writing them is what re-cast the diagnosis from "systemic RLS bug" to "client never reconciles on mount." The mandated coverage didn't just prevent the next bug; it root-caused this one.
