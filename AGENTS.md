...
**New framework directory: `compass/scripts/`.** Reference utility scripts that complement workflows. Each script single-file + stdlib-only + PM-operable. First script: `token-usage.py` (per-role token rollup). Add scripts here only when problems are structurally hard to solve with markdown docs alone.
