...
**Scope-discipline pattern — when Compass would need to build an integration with external tools/agents/services, declare the pattern + registry + manual fallback instead of writing the integration.** Per `[declare-not-implement]` (v0.3.9, `compass/framework/canon.md`). **First scope-discipline class Compass-original; introduces a 6th pattern shape.** Catalog now spans 6 shapes / 10 patterns: enforcement (4) · interaction (1) · freshness (1) · observability (1) · handoff (2) · scope-discipline (1). The first 5 shapes govern how workflows execute; scope-discipline governs Compass's own scope at framework design time. Two instances at codification: (1) v0.3.5 `[agent-handoff]` — declared 5-piece handoff shape + shipped template with 4 commented reviewer blocks; consumer wires per-CLI integration. (2) v0.3.8 same-day correction — declared `agents:` registry + delegated API-based-agent adapter layer to upstream libraries (LiteLLM / Vercel AI SDK / OpenRouter / LangChain); refused to ship per-agent adapter docs that would duplicate upstream. **Anti-pattern closed: `integration-creep`** — integration scope grows linearly with integration count; Compass-maintainer scope does not. Invoke at release planning, codification, or roadmap-deferral decisions. **Load-bearing oversight is the user** — v0.3.8's same-day correction was caught by user, not framework alone; the framework's `[declare-not-implement]` reflex is real but not infallible.
