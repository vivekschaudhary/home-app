...
| SUP-7 | 2026-06-15 | dogfood | "change password page… enter the password and save, gives a 502 on the post" | bug | P1 | **re-opened** | `/fix` → #52 (error discrimination) **then** the real cause: prod logs showed `insufficient_aal` (MFA account needs AAL2 to reset) → v2 adds the authenticator step to recovery (PR #54) |
