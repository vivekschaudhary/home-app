...
| SUP-7 | 2026-06-15 | dogfood | "change password page… enter the password and save, gives a 502 on the post" | bug | P1 | shipped (verify live) | `/fix` → #52 (error discrimination) **then** the real cause from prod logs: `insufficient_aal` (MFA account needs AAL2) → v2 adds the authenticator step to recovery (PR #54, deployed). Awaiting reporter's live confirmation. |
