...
- **`counts_as_spending` is GENERAL + per-user, not transfer-only.** The column lives on **every** `categories` row (default `true`) and is read per-user (like `kind`/`isEssential`) — so the schema supports a user excluding *any* of their own categories (a custom "Family loan", "Brokerage funding", etc.) from day one. Build it as a general per-category property; the protected "Transfers & Payments" category is merely the **seeded default home** with the flag pre-set to `false`. Do NOT hardcode the exclusion to the one bucket. ([[user-categories-are-source-of-truth]] — each user's category set + flags are theirs.)
- **Out of scope (this slice):** changing recap/anomaly's *uncapped main reads* (the separate logged FIX-2026-06-20c follow-up) — the engineer MAY fold the one-line `readAllPaged` swap in while touching those files, but it is not an AC here. A UI to toggle `counts_as_spending` on an **arbitrary** custom category — the *data model* supports it now, but the toggle affordance rides the deferred **category-management** surface (rename/edit/delete, deferred since WLT-22-2); this slice only needs the protected bucket + move-transactions-in/out. Fast-follow, not a blocker.
