...
- [ ] **AC3 — auto-assignment (default-correct, non-destructive):** transfers and credit-card payments are auto-assigned to the protected category by writing `transaction_categories(assigned_by='system')` for every matching transaction that has **no existing `'user'` assignment** — matched on Plaid `TRANSFER_IN` / `TRANSFER_OUT` (primary) and the credit-card-payment detailed category (`LOAN_PAYMENTS_CREDIT_CARD_PAYMENT`). **`LOAN_PAYMENTS` other than CC-payment (mortgage/auto/student) stays spending.** Runs on first load over history AND on Plaid sync for new rows (so it stays correct over time). Paginated past the 1000-row cap.
...
- **Seed + auto-assign** in the existing `ensureSeededCategories` path (`app/lib/categories.ts`) + the Plaid sync job: seed the one `source='system'` "Transfers & Payments" category; resolve which transactions are transfers/CC-payments from the **stored Plaid category** (primary `TRANSFER_IN`/`TRANSFER_OUT`; the CC-payment detailed key — note `transactions.category` stores the **primary** today, so the detailed CC-payment signal may need the detailed category surfaced at ingest — confirm in build, else fall back to `LOAN_PAYMENTS` primary with a documented caveat). Write `transaction_categories(assigned_by='system')` only where no `'user'` row exists; **paginate** the read (`readAllPaged`, the FIX-2026-06-20c helper). Extend the `assigned_by` check to include `'system'`.
- **Precedence:** `'user'` > `'system'` (auto) — a user override always wins and is never clobbered by re-seeding or sync. Mirrors the `overrideUserAssignments` discipline from FIX-2026-06-20.
...
### Risks
- [2026-06-20] [PM] **`transactions.category` stores Plaid's PRIMARY, so the CC-payment *detailed* signal may not be present** to distinguish a card payment from a mortgage — likelihood: medium — impact: medium — mitigation: surface the detailed category at ingest, OR exclude `LOAN_PAYMENTS` primary with a documented caveat + rely on user override; confirm in build — area: data
