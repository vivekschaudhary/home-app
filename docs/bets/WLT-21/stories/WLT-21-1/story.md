...
- [ ] **AC4 — Recommended from history (honest).** Per-category figure from the user's own trailing monthly spend — **median of monthly totals**, a modest **10% trim on discretionary** categories (essentials untrimmed: `RENT_AND_UTILITIES, FOOD_AND_DRINK, GROCERIES, LOAN_PAYMENTS, MEDICAL, INSURANCE, TRANSPORTATION` — covering both the Plaid-primary taxonomy and the legacy/documented names the mapper can emit, so groceries-as-`FOOD_AND_DRINK` OR `GROCERIES` is untrimmed either way). **Cold-start** (<~1 month of history for that category) → "—" + the one-line history note; **never a fabricated number**.
