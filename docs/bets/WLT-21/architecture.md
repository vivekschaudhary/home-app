...
- `computeRecommendedBudgets(txns, asOf): Map<string, number>` — per category, sum debits per **calendar month** across a **trailing ~6 months**, take the **median of those monthly totals** ("typical"); recommend `typical` for **essential** categories (`RENT_AND_UTILITIES, GROCERIES, LOAN_PAYMENTS, MEDICAL, INSURANCE`) and `0.90 × typical` (modest 10% trim, the floor) for discretionary. **≥1 month of history required** → else no recommendation (honest "—").
