...
- **WLT-12 (Assemble + run your first workflow — net-worth snapshot) — `ready`.** The engine end-to-end + the first archetype (`networth_snapshot`, covers 5 goalKinds, balances-only → lowest data risk): declare → connect bridge → personalized net-worth snapshot → "set your target" action → `WorkflowRun` (first WAWU event). Other 9 goalKinds keep the WLT-11 placeholder until their archetype story (total-coverage = bet exit gate). **Next: `/build WLT-12`.**
