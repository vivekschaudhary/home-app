...
- **Roadmap clarification.** v0.3.8 ships L1 (pattern + registry + defaults). **L2 deferred to v0.3.9** — per-agent adapter docs at `compass/agents/<agent>.md` describing invocation, auth, context-loading, role-prompt location per agent. **L3 deferred to v0.3.10** — `compass/scripts/setup-agent.py` propagation script reading `tool_assignments` and generating per-agent prompt directories. Together these resolve the 3-surface drift structurally; v0.3.8 ships the registry that the later layers consume.
